...
const updateProgress = reactive({
    ...
    next_step: null,
    percentage: 0,
    message: 'در حال بررسی وضعیت سیستم...',
    isError: false,
});

function sendUpdateRequest() {
    updateForm.post(route('admin.backup.restore'), {
        showProgress: false,
        onStart: () => {
        ...
        onSuccess: (page) => {
            ...
            if (step === 'finished') {
                window.location.href = route('admin.backup.list');
                return;
            }
...
<template>
    <Head title="بازگردانی فایل پشتیبان" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            <AppPageTitle
                icon="DatabaseBackup"
                title="بازگردانی فایل پشتیبان"
                subtitle="در این صفحه می توانید وضعیت بازگردانی فایل پشتیبان را مشاهده کنید."
            >
            </AppPageTitle>
            ...
            <div class="mt-4">
                ...
                <div v-if="updateProgress.isError" class="mt-4 flex items-center justify-center">
                    <Button variant="outline" @click="router.visit(route('admin.backup.list'))">
